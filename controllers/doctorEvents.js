...
const {
  ...
  eventsSchema,
  appointmentsSchema,
} = require('../models/calendarEventsSchema')
  ...
  try {
    ...
    for (let i = 0; i < hospitals_days_arr.length; i++) {
      ...
      let start_time_value =
        parseInt(start_time_arr[0]) * 60 + parseInt(start_time_arr[1])
      let end_time_value =
        parseInt(end_time_arr[0]) * 60 + parseInt(end_time_arr[1])
  ...
  try {
    ...
    // for else condition only change the reel matrices not the real matrices
      ...
      if (today.getDate() != totalDays) {
        ...
        for (
          let i = tomorrow.getDate() - 1;
          i < new_reel_matrix.length - 1;
          i++
        ) {
        ...
        await matrix
          ...
          .then(() => {
            console.log(
              `Matrix updated for ${existing_matrices[0].matrix_date}`
            )
          })
          .catch((err) => {
            console.log(err.message)
      ...
      for (let i = 1; i < existing_matrices.length; i++) {
        ...
        await matrix
          ...
          .then(() => {
            console.log(
              `Matrix updated for ${existing_matrices[i].matrix_date}`
            )
          })
          .catch((err) => {
            console.log(err.message)
...
const addEventToMatrix = async (
  ...
  try {
    // the date format is already in IST so no need to convert it and hence always store the appointment_date in IST format
    const event_time = new Date(event_start)
    const matrix_date = new Date(event_start)
    matrix_date.setUTCHours(0, 0, 0, 0)
    matrix_date.setUTCDate(1)
    console.log(matrix_date)

    const matrix_row = event_time.getUTCDate() - 1
    const start_index =
    ...
    console.log(matrix_row,start_index,end_index)
    const matrix = mongoose.model('eventMatrices', eventMatrixSchema)
    ...
    const new_matrix = matrix_details[0].event_matrix.map((arr) => arr.slice())

    for (let i = start_index; i <= end_index; i++) {
      new_matrix[matrix_row][i] = event_id
    }

    await matrix.findOneAndUpdate(
      { _id: matrix_details[0]._id },
      {
        event_matrix: new_matrix,
      }
    )

    console.log(`${matrix_type} matrix updated successfully`)
    res.status(200).json({ message:`${matrix_type} matrix updated successfully`})
  } catch (err) {
    console.log(err.message)
    res.status(500).json({ message: err.message })
  }
...
const startNewEvent = async (req, res) => {
  try {
    const doctor_id = req.params.doctor_id
    const new_event_id = req.body.event_id

    const doctor = mongoose.model('details', detailsSchema)
    ...
    //const details=await doctor.findOne({doctor_id});
    console.log(details)
    res.status(200).json({ message: 'New event started successfully', details })
  } catch (err) {
    console.log(err.message)
    res.status(500).json({ message: err.message })
  }
...
const endCurrentEvent = async (req, res) => {
  const doctor_id = req.params.doctor_id
  const event_id = req.body.event_id
  const is_event_completed = req.body.is_event_completed //true if the appointment/event is done or false if the appointment/event is cancelled
  const reason = req.body.reason
  const now = new Date(new Date().getTime() + 5.5 * 60 * 60 * 1000)

  const doctor = mongoose.model('details', detailsSchema)

  const doctor_details = await doctor.findOne({ doctor_id })
  const doctor_old_event_id = doctor_details.doctor_current_event.event_id
  const doctor_old_event_start_time = doctor_details.doctor_current_event.from

  if (doctor_old_event_id == null) {
    res.status(400).json({ message: 'No event is currently running' })
  } else if (event_id != doctor_old_event_id) {
    res
      .status(400)
      .json({
        message: `The event_id doesn't match with the current event_id,so first complete the event ${doctor_old_event_id}`,
      })
  } else {

    // update the current event slot
    const details = await doctor.findOneAndUpdate(
      { doctor_id },
      {
        doctor_current_event: {
    ...
    // updating the event and appointment models
    const event = mongoose.model('events', eventsSchema)
    const appointemnt = mongoose.model('appointments', appointmentsSchema)
    // the event_id and appointment_id should be already present in the database
    await event.findOneAndUpdate(
      { _id: doctor_old_event_id },
      {
        is_event_completed: {
          type: is_event_completed,
          reason,
        },
      }
    )
    // await appointemnt.findOneAndUpdate(
    //   { _id: doctor_old_event_id },
    //   {
    ...
    //   }
    // )
    console.log('Old event updated successfully')
    //add the current event to the event matrix
    let event_duration = (now - doctor_old_event_start_time) / 60000
    // making event_duration integer
    event_duration = Math.round(event_duration)
    //why 60000 because the difference is in milliseconds and we need to convert it into minutes
    await addEventToMatrix(
      doctor_id,
      doctor_old_event_start_time,
      event_duration,
      event_id,
      'real',
      res
...
const unScheduledEvent = async (req, res) => {
  try {
    ...
    if (event_type == 'E') {
      //i.e any emergency patient or situation needed to be handled by the doctor,only inform those patients of that day
      const today = new Date(new Date().getTime() + 5.5 * 60 * 60 * 1000)
      const matrix = mongoose.model('eventMatrices', eventMatrixSchema)
      ...
      const matrix_details = await matrix.find({
        doctor_id,
        matrix_date: { $gte: firstDayOfTheMonth, $lte: firstDayOfTheMonth },
      })
